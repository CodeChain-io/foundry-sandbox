...
impl TransportRecv for UdpRecv {
    fn recv(&self, timeout: Option<std::time::Duration>) -> Result<Vec<u8>, TransportError> {
        ...
        let (size, addr) = self
            .socket
            .recv_from(self.buffer.try_borrow_mut().unwrap().as_mut_slice())
...
impl Ipc for Udp {
    ...
    fn arguments_for_both_ends() -> (Vec<u8>, Vec<u8>) {
        ...
        (
            serde_cbor::to_vec(&(port1, port2)).unwrap(),
            serde_cbor::to_vec(&(port2, port1)).unwrap(),
        )
    }
    ...
    fn new(data: Vec<u8>) -> Self {
        ...
        let socket =
            Arc::new(UdpSocket::bind(format!("127.0.0.1:{}", port1)).expect("Port is in use"));
        Udp {
            send: UdpSend {
